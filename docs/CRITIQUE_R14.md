...
| 5 | Rider is sitting in a chair — pelvis at deck height, thighs horizontal | `absorb+tuck+compress+grab` sums to 0.72 against `standH` 0.735. The guard trims `hx`, which only reaches pelvis height via sin(roll) — zero authority on a flat board | **fixed (unverified)** |
...
| 10 | Rider is eight shades of one charcoal — ~10-level spread head to foot | `shell` and `shellGrey` shipped as **byte-identical hex** `0x2c2e33`, so the yoke/sleeve blocking was a no-op | **fixed (unverified)** — jacket now `#E8531F` per §6.3 |
...
| 15 | Rock props are detached floating slabs, sky visible under them | `buildSlabStack` random-walks `ox` by ±0.20 cumulatively over 5 slabs against a 0.5 plan radius, then draws the underside | **open** |
